...
class Video extends createjs.Container {
  constructor(args = {}) {
    ...
    var isVideoStopped1 = false;
    var isVideoStopped2 = false;
    ...
    video.ontimeupdate = () => {
      ...
      if (video.currentTime > 5.1 && !isVideoStopped1) {
        isVideoStopped1 = true;
        this.stop();
      }
      if (video.currentTime > timeValue && !isVideoStopped2) {
        isVideoStopped2 = true;
        onTimeOut(video.currentTime);
      }
    };
  }
  ...
  */
  _initButtons() {
    const soundButtons = new SoundButtons({
      action: (doMute) => (this.video.muted = doMute),
    });
...
module.exports = createjs.promote(Video, 'Container');
