...
const DEFAULT_VALUES = {
  ...
  fadeObjects: [], // [Array of Array of Strings] contains the name of the fadeObject items
  ...
  name: "carousel", // [function] callback fadeObjects are visible, param is an Array of the fadeObjects
...
const { map, keepSafe, logError } = require("./helper");

const images = ad.assets.images;

const TARGET_DETECTION = 0.005;

const Track = require("./trackings.js");
const Video = require("./video");
const SoundButtons = require("./soundButtons");
const { LAYOUT_2D, LAYOUT_3D, LAYOUT_ANDROID } = require("./const");
const track = new Track();

class Carousel extends createjs.Container {
  constructor(args = {}) {
  ...
  setupInteractions() {
    this.on("mousedown", (e) => {
      ...
      this.mouseStart = this.mouseLast0 = this.mouseLast1 =
        e[this.isVertical ? "localY" : "localX"];
    });
    ...
    this.on("pressmove", (e) => {
      ...
      e.stopPropagation();
      e.stopImmediatePropagation();
      const { startIndex, mouseStart, mouseLast1 } = this;
      this.currIdx =
        startIndex +
        (mouseStart - e[this.isVertical ? "localY" : "localX"]) /
          (this.isVertical ? this.imageHeight : this.imageWidth);
      this.mouseLast0 = mouseLast1;
      this.mouseLast1 = e[this.isVertical ? "localY" : "localX"];
      if (!this.isCyclic) {
    ...
    this.on("pressup", (e) => {
      ...
      if (mouseStart === e[this.isVertical ? "localY" : "localX"]) {
  ...
  update3d() {
    ...
    for (let i = 0; i < nbMedia; i++) {
      ...
      if (isVertical) {
        ...
        currImage.y = height / 2;
      }
      let d = 1;
      if (angle < PI / 2) {
        d = map(angle, 0, PI / 2, 0, 1);
      } else if (angle > (3 * PI) / 2) {
        d = map(angle, TWO_PI, (3 * PI) / 2, 0, 1);
      }
  ...
  updateAndroid() {
    const { scaleUnfocus, scaleFocus, fadeObjects, currIdx, nbMedia } = this;
    ...
    for (let i = iMin; i < iMax; i++) {
      ...
      if (d > -1 && d <= 0) {
        ...
        opacity = map(d, 0, 0.5, 1, 0);
        magnify = map(d, 0, 0.5, scaleFocus, scaleUnfocus);
      }
      currImage.x = x;
      currImage.y = y;
      currImage.alpha = opacity;
    ...
    this.allMedia = mediaUrls.map((imageUrl, i) => {
      if (imageUrl.includes(".mp4")) {
        const video = new Video({
          ...
          regX: layout === LAYOUT_3D ? 0.5 : 0,
          regY: layout === LAYOUT_2D ? 0 : 0.5,
        });
        ...
        return new Array(this.nbClones).fill(0).map((_, i) => {
          ...
          clone.index = i;
          clone.regX = layout === LAYOUT_3D ? asset.image.width / 2 : 0;
          clone.regY = layout === LAYOUT_2D ? 0 : asset.image.height / 2;
          return clone;
        });
      }
    });
...
module.exports = createjs.promote(Carousel, "Container");
